...
- **מבנה Firestore:** מסמך-מטא `users/{uid}/attachments/{id}` = `{name,type,size,mime,chunks:N,createdAt}`
  ...
  קיימים. **תקרה:** `CloudFiles.FS_MAX`=50MB (מעבר → מקומי) — מיושר ל-HARD_CAP של media.js,
  כך שכל קובץ שהמחברת מקבלת בכלל מגיע לענן. **קריאה:** `fetchBlob` (המסלול לקבצים גדולים)
  מפענח part-אחרי-part ל-Blob במקום לשרשר מחרוזת base64 של הקובץ כולו. ⚠️ `_readParts` קורא
  part ריק שוב מהשרת (`source:'server'`) ו-`_assertLen` מאמת את האורך מול `meta.size` —
  בלי זה part חסר הפך בשקט לקובץ קצר-אך-"תקין" (ראה UPDATES 26.8).
...
ו-rebuild באמצע קורא גוף ישן ומעלה שוב בלעדיו. מדלג על כרטיסים ריקים (העותק-החתוך) ומעל `FS_MAX`.
...
**הקלטת וידאו מחלון/טאב (31.7.2026, memos v=14):** כפתור "🎥 הקלט וידאו מהחלון" + בורר משך
(5 דק׳→3 שעות; ברירת מחדל 3ש = `VID_MAX_SEC`, או "ללא הגבלה") — בכרטיס העברי בלבד (לא תלוי-שפה).
`startTabVideoRec` מקליט את **כל** ה-stream של `getDisplayMedia({video,audio})` (וידאו+קול), ובסוף
המשך שנבחר `setTimeout` קורא ל-`stopVideoRec` → הקובץ נסגר ונשמר **אוטומטית**.
⚠️ **היעד הוא קובץ בדיסק ולא IndexedDB** (3ש ≈ ג׳יגה-בייטים; אין תמלול/ניגון-פנימי לווידאו):
מסלול מועדף **File System Access בזרימה** — `showSaveFilePicker` נקרא **לפני** `getDisplayMedia`
(שתיהן דורשות user-gesture ו-getDisplayMedia מכלה אותו!), והנתחים נכתבים תוך כדי דרך שרשרת-כתיבה
טורית (`_vWriteChain` — אסור לכתוב במקביל לאותו writable) ⇒ אפס גידול-זיכרון וקובץ ששורד קריסה;
**נפילה** = צבירת Blob-ים + הורדה אוטומטית לתיקיית ההורדות. MP4 מועדף (`pickVideoMime`), אחרת WebM.
מצב הווידאו **נפרד לחלוטין** ממצב השמע (`_vmr`/`_vCards`/`_vLimitSec`/`.vm-vid-pill`), והשניים
**בלעדיים זה לזה** — כל צד מסתיר את פקדי השני ב-`setRecording`. `track.ended` ("הפסק שיתוף" של
הדפדפן) → `stopVideoRec` (שומר, לא מאבד).
**דו-לשוני (31.7.2026, memos v=15):** הכפתור קיים ב**שני** הכרטיסים, כל אחד בשפתו — תוויות
ואפשרויות-משך מתורגמות, ושם הקובץ נגזר מהכרטיס שהתחיל (`_vLang`, מקביל ל-`_recLang`).
`_vCards={he,en}` (מקביל ל-`_cards`) ו-`setVidRecordingAll(on)` מעדכן את שניהם: המתחיל מציג "עצור",
השני **מושבת** (`setRecording(mine, otherBusy)`). ⚠️ **מלכודת UX מתועדת:** המתג "שיתוף גם של האודיו
מהכרטיסייה" בחלון בחירת-המקור **כבוי כברירת מחדל** — בלעדיו הסרטון יוצא ללא קול. לכן יש שלוש
תזכורות: `.vm-vid-hint` קבועה ליד הכפתור, שלב `.vm-audio-warn` מודגש במדריך, ו-toast ייעודי
כשה-stream חוזר בלי audio track. אל תסיר אותן — זו התקלה הנפוצה ביותר בפיצ'ר.
