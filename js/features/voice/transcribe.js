(function () {
  ...
  // ⚠️ timeout חובה (15.7): בלי abort, בקשת-תרגום תלויה הייתה תוקעת את כל
  // זרימת-האנגלית לנצח אחרי שהתמלול כבר הצליח — אותה מחלקת-באג כמו תקיעת
  // ההעלאות. timeout ⇒ נזרק ⇒ googleOk=false ⇒ MyMemory ⇒ במקרה הגרוע
  // התרגום מדווח ככושל אבל התמלול תמיד חוזר.
  ...
  // תוחם promise חיצוני (PTR_ENGINE של P-43 — לא נוגעים בקובץ שלו) בזמן:
  // המפסיד ממשיך לרוץ ברקע, אבל אנחנו לא נתקעים עליו.
  ...
  async function translateToHebrew(text, onProgress) {
    ...
    for (var i = 0; i < parts.length; i++) {
      ...
      if (t === null && window.PTR_ENGINE && PTR_ENGINE._translatePageText) {
        try {
          var m = (await _withTimeout(PTR_ENGINE._translatePageText(parts[i]), 20000, 'MyMemory timeout') || '').trim();
