...
${gratitude ? `<div>
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  //  WORD IMAGE EMBEDDING — MHTML (permanent fix, do NOT revert to data-URIs)
  // ═══════════════════════════════════════════════════════════════════════════
  //  WHY THIS EXISTS: exporting Word as an HTML blob with <img src="data:...">
  //  base64 data-URIs is UNRELIABLE — many Word builds render such images as a
  //  broken "linked image" frame ("אין אפשרות להציג את התמונה המקושרת"). It works
  //  on some machines and not others, which is why the bug kept "coming back":
  //  it's the FORMAT that's flaky, not a one-off code bug. PDF is unaffected
  //  because it rasterises everything.
  //
  //  THE FIX: when the document contains images, emit an MHTML (multipart/related)
  //  file — the exact format Word itself produces for "Single File Web Page".
  //  The HTML references each image by a relative name (image001.png); each image
  //  is a separate base64 MIME part whose Content-Location matches. Word embeds
  //  these natively and reliably on every build. Text-only docs keep the simple
  //  HTML-as-.doc path (that path was never broken).
  //
  //  ⚠️ FUTURE AGENTS: do not "simplify" this back to inline data-URI <img> in
  //  the Word blob — that is the exact regression this replaces. See ARCHITECTURE
  //  §6 (Word image embedding). Verify by re-opening an exported .doc that has a
  //  pasted screenshot IN WORD, not just in a browser.
  function _mimeExt(mime) {
    var map = { 'image/png': 'png', 'image/jpeg': 'jpg', 'image/jpg': 'jpg',
      'image/gif': 'gif', 'image/webp': 'webp', 'image/bmp': 'bmp', 'image/svg+xml': 'svg' };
    return map[(mime || '').toLowerCase()] || 'png';
  }
  // Re-wrap a base64 string into 76-char lines (RFC 2045 line-length limit).
  function _chunk76(s) {
    var out = []; for (var i = 0; i < s.length; i += 76) out.push(s.slice(i, i + 76));
    return out.join('\r\n');
  }
  // Quoted-printable encode a JS string's UTF-8 bytes. Everything except printable
  // ASCII becomes =XX (spaces/tabs/newlines included) so there is never a trailing-
  // whitespace or hard-line-break ambiguity — only QP soft breaks (=CRLF).
  function _qpEncode(str) {
    var bytes = new TextEncoder().encode(str), out = '', lineLen = 0;
    for (var i = 0; i < bytes.length; i++) {
      var b = bytes[i];
      var tok = (b >= 33 && b <= 126 && b !== 61)
  ...
  // Assemble the MHTML envelope. htmlDoc = full <html>…</html>; imgParts =
  // [{fname, mime, b64}]. img srcs in htmlDoc are the same relative fnames.
  function _buildMhtml(htmlDoc, imgParts) {
    var B = '----=_NextPart_HAMACHBERET_MHT', CRLF = '\r\n', o = '';
    o += 'MIME-Version: 1.0' + CRLF;
    o += 'Content-Type: multipart/related; boundary="' + B + '"; type="text/html"' + CRLF + CRLF;
    o += 'This is a Web Archive (single file) generated by המחברת שלי.' + CRLF + CRLF;
    o += '--' + B + CRLF;
    o += 'Content-Location: file:///C:/doc.htm' + CRLF;
    o += 'Content-Type: text/html; charset="utf-8"' + CRLF;
    o += 'Content-Transfer-Encoding: quoted-printable' + CRLF + CRLF;
    o += _qpEncode(htmlDoc) + CRLF;
    imgParts.forEach(function (p) {
      o += '--' + B + CRLF;
      o += 'Content-Location: file:///C:/' + p.fname + CRLF;
      o += 'Content-Type: ' + p.mime + CRLF;
      o += 'Content-Transfer-Encoding: base64' + CRLF + CRLF;
      o += _chunk76(p.b64) + CRLF;
    });
    o += '--' + B + '--' + CRLF;
    return o;
  }
  ...
  async function exportDoc(topic, editor, format) {
    ...
    // ── Word export ───────────────────────────────────────────────────────────
    if (format === 'word') {
      // Move every inline data-URI image OUT of the HTML and into its own MHTML
      // part (see the MHTML block above for why). Re-serialise AFTER this so the
      // HTML references relative filenames instead of huge base64 blobs.
      const imgParts = [];
      let imgN = 0;
      cloned.querySelectorAll('img').forEach(im => {
        const src = im.getAttribute('src') || '';
        const m = /^data:([^;,]+)?(;base64)?,([\s\S]*)$/i.exec(src);
        if (!m) return;                              // external URL — leave as link
        const mime = m[1] || 'image/png';
        let b64;
        if (m[2]) { b64 = m[3].replace(/\s+/g, ''); }           // already base64
        else { try { b64 = btoa(unescape(m[3])); } catch (e) { return; } } // URL-encoded (e.g. SVG)
        imgN++;
        const fname = 'image' + ('00' + imgN).slice(-3) + '.' + _mimeExt(mime);
        im.setAttribute('src', fname);
        imgParts.push({ fname, mime, b64 });
      });

      const bodyForWord = cloned.innerHTML;   // now references image001.png etc.
      const html = [
        ...
        `<body dir="rtl" style="font-size:11pt;font-family:Arial,sans-serif;"><div class="Section1"><h1>${title}</h1>${bodyForWord}</div></body></html>`
      ].join('');

      // Images present → MHTML (Word embeds them reliably). None → the proven
      // plain HTML-as-.doc path (was never broken; avoids MHTML overhead).
      const blob = imgParts.length
        ? new Blob([_buildMhtml(html, imgParts)], { type: 'application/msword' })
        : new Blob(['﻿', html], { type: 'application/msword' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url; a.download = title + '.doc'; a.click();
      setTimeout(() => URL.revokeObjectURL(url), 2000);
      App.toast(imgParts.length ? '✓ קובץ Word הורד (' + imgParts.length + ' תמונות מוטמעות)' : '✓ קובץ Word הורד');
    }
  }
