...
## עיצוב הדוח — צמוד לתבנית הסקיל
הדוח מרונדר ב**תבנית ה-HTML/CSS המדויקת של הסקיל** (`wyckoff-BTC_USDT-*.html`):
header+verdict לחיץ, טבלת רב-טווחים (שבועי/יומי/4H/1H/15m), "⚖️ מי שולט" עם
ctrl-tag + גרף לכל TF, תרחישים (scen long/short), "🎯 מסקנה" (opt+טבלה), צ'ק-ליסט
לחיץ → מודאל QDATA, "🚫 מה מבטל הכל", ייצוא PDF/HTML. ⚠️ ה-CSS של הסקיל גלובלי
(body/h2/table) — לכן בתצוגה החיה הוא **ממוקד תחת `.wk-report`** ב-wyckoff.css
(אחרת שובר את האתר); גרסה לא-ממוקדת מוטמעת ב-index.js (`EXPORT_CSS`) לקובץ העצמאי.
הגרפים מוטמעים כ-PNG (canvas→toDataURL), כמו דוח הסקיל המקורי.
**כפתור דשבורד** "📈 דוח וויקוף עצמאי" (BTC/ETH/SOL) נדחף ל-`window.DASHBOARD_WIDGETS`
(הרחבה רכה, אפס נגיעה ב-dashboard/wyckoff הקיים).
