...
- **⚠️ תלות חיצונית:** דורש כללי-הרשאה ב-Firebase Storage שמתירים כתיבה ל-`users/{uid}/attachments/` למשתמש מחובר; אם ההעלאה נכשלת → נפילה מקומית + toast עם קוד. **אסור לגעת בנתיב שמירת התמונות** (media.js insertImage/Editable) — הוא לא קשור לקבצים.
