...
- **מהות:** המשפך היחיד לכל תמונה באתר (מחברת/הערות/סקירה): הדבקה, figures, גרירה, ידיות-גודל, snap, מדיניות איכות (≤150KB=ביט-בביט; מעל=סולם-דחיסה). **וגם (23.7):** ייבוא-Word-לפתק 📥 (docx/MHTML לפי חתימת-בייטים), תמונות-פתק-כבדות→ענן (data-fsimg + hydration + imgcache) וריפוי-קבצים-מקומיים→ענן — הכול מפורט ב-ARCHITECTURE §5ב. **קבצים מצורפים** (לא-תמונה): מאוחסנים ב-**Firestore מפוצל ל-chunks** (הנושא שומר `data-fs` בלבד → מסתנכרן מלא, זמין מכל מכשיר), עם נפילה ל-base64 מקומי אם לא-מחובר.
...
- **בוצע (26.8.2026, cloud-files v=7 · media v=63):** 📚 **קבצים עד 50MB + הורדה שלא משקרת.** `FS_MAX` הועלה 20MB→50MB (מיושר ל-HARD_CAP של media.js) — קודם קובץ של 38-49MB נפל בשקט ל-base64 מקומי, כלומר "יש אצלי ואין במחשב השני". נוסף `CloudFiles.fetchBlob`: מפענח כל part בנפרד (CHUNK כפולה של 4 ⇒ כל part הוא base64 תקף בפני עצמו) ובונה Blob ממערכי-בייטים — במקום להחזיק ~67MB base64 + עותק משורשר + מחרוזת בינארית בו-זמנית (מקפיא/מפיל את הטאב). ⚠️ **באג-אמת שנתפס בייצור:** part שחזר "לא קיים" הוחלף במחרוזת ריקה ⇒ **PDF קצר יותר שנפתח יפה אבל חסרים בו עמודים** (נצפה: 19MB במקום 38MB). השורש: הקריאות נענו מהמטמון המקומי של Firestore (~40MB, מפנה מסמכים אחרי העלאה כבדה). **תוקן:** `_readParts` קורא part ריק שוב עם `source:'server'` וזורק `file/part-missing` אם עדיין חסר; `_assertLen` מאמת סכום-אורכים מול `4*ceil(meta.size/3)`; `fetchBlob` מאמת גם `blob.size===meta.size`. **שני המסלולים נכשלים ברעש במקום למסור קובץ חלקי.** אומת: 3 ספרים (46.2/46.7/36.8MB) הועלו לפתק "ספרים" ו-SHA-256 של כל אחד אחרי סבב-ענן מלא **זהה למקור**.
...
- **בוצע (31.7.2026, memos v=14 · voice.css v=10 · knowledge v=21):** 🎥 **הקלטת וידאו מחלון/טאב + הגבלת-זמן.** כפתור "🎥 הקלט וידאו מהחלון" (כרטיס עברי בלבד — לא תלוי-שפה) מקליט וידאו+שמע של חלון/כרטיסייה/מסך, עם בורר **משך** (5 דק׳→3 שעות, ברירת מחדל 3ש = התקרה, או "ללא הגבלה") שבסיומו ההקלטה **נעצרת ונשמרת אוטומטית**. ⚠️ **היעד הוא קובץ בדיסק, לא IndexedDB** (3ש וידאו ≈ ג׳יגה-בייטים): מסלול מועדף = File System Access בזרימה (ה-picker נקרא **לפני** getDisplayMedia כי הוא מכלה את ה-user-gesture) → אפס גידול-זיכרון; נפילה = צבירה בזיכרון + הורדה אוטומטית להורדות. MP4 כשנתמך, אחרת WebM. שלט צף נפרד (`.vm-vid-pill`) עם "עבר/מתוך". **בלעדיות מול הקלטת שמע** (הצד השני מוסתר). מדריך 5 שלבים ב-hover.
- **בוצע (31.7.2026, memos v=15 · voice.css v=11):** 🌐 **הווידאו בשני הכרטיסים, כל אחד לפי שפתו** (בקשת המשתמש: "בחלק העליון הכל בעברית, בתחתון הכל באנגלית"). תוויות הפקדים ואפשרויות המשך מתורגמות פר-כרטיס, ושם הקובץ נגזר מהכרטיס (`הקלטת מסך …` מול `Screen recording …`) דרך `_vLang` — בדיוק כמו `_recLang` בשמע. `_vCards` מחזיק את שני ה-ui (מקביל ל-`_cards`): הכרטיס שהתחיל מציג "עצור", כפתור הכרטיס השני **מושבת**, ופקדי השמע מוסתרים בשניהם. בנוסף — **מתג "שיתוף גם של האודיו מהכרטיסייה" הובלט** (כבוי כברירת מחדל = הסיבה הנפוצה לסרטון בלי קול): תזכורת-ענבר קבועה ליד הכפתור, שלב מודגש במדריך, ו-toast שמנקב בשם המתג כשלא הגיע פס-קול.
- **⏳ משימה פתוחה (תוכנית מאושרת-לתכנון, 31.7.2026) — שילוב וידאו + תמלול:** היום הקלטת-וידאו נשמרת
  רק כקובץ ואי-אפשר לתמלל אותה. התוכנית: **שכבה 1** — מקליט-אודיו שני במקביל שיוצר **memo רגיל**
  (כך שכל פונקציות התמלול עובדות בלי שינוי) + שדה `memo.videoFile`; **שכבה 2** — תמלול חי בחלונות
  90ש דרך ברז Web-Audio + `startLive` חדש ב-transcribe.js (בלי לגעת ב-`run()`). ⚠️ מלכודות: לעצור
  tracks רק אחרי ששני המקליטים סיימו · נתחי MediaRecorder אינם עצמאיים (חובה PCM) · לשונית "חלון"
  = בלי שמע. **פירוט מלא + נימוקים: רשומת UPDATES 31.7.2026 "תוכנית מאושרת-לתכנון".**
  **טרם אושר למימוש — לשאול את המשתמש במה להתחיל.**
- **זהירות (commit 65107d3):** הקלטה רציפה חיה ברמת המודול; `hashchange` עוצר ניגון בלבד — **אסור להחזיר לשם stopRec()**. שלט צף `.vm-rec-pill` בכל עמוד. מאז יולי 2026 המצב החי הוא `_cards`+`_recLang` (לא `_ui`); הקלטה אחת פעילה בכל רגע בשתי הרובריקות. מצב הווידאו נפרד לגמרי (`_vmr`/`_vUi`/`_vLimitSec`) — **אל תערבב בין המנועים**.
