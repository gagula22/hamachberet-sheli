...
- **בוצע (15.7.2026, transcribe v=9):** 🛡️ **עמידות להקלטות ארוכות (3 שעות ≈ 120 נתחים).** `cloudChunkedParallel` קיבל **ניסיון-חוזר per-נתח** (3 ניסיונות, backoff 2.5/5ש) + **המשך-חלקי** — נתח שנכשל סופית מדולג (119/120 במקום כלום), רק כשל-כולל זורק→מקומי. הנתחים החסרים מדווחים (progress + תווית מנוע). ⚠️ מגבלה שנותרה: פענוח PCM = buffer יחיד (~1.5GB שיא ל-3ש) — תלוי-מכשיר, לא תוקן (נגיעה גדולה, לא דווח ככשל בפועל).
